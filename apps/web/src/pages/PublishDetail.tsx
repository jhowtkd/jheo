...
const STATUS_HINT: Record<PublishStatus, string> = {
  queued: 'Waiting for a worker slot.',
  running: 'Worker is publishing to the channel.',
  completed: 'Done — check the external URL.',
  failed: 'All retries exhausted. Inspect lastError and retry.',
    ...
    <div className="page">
      <div className="page__header">
        <div>
          <div className="row" style={{ marginBottom: 'var(--space-2)', gap: 'var(--space-2)' }}>
            ...
            <span className="tiny">publish</span>
          </div>
        ...
        <div className="status-meta">
          ...
          <span className="tiny tabular muted">attempts {p.attempts}</span>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 'var(--space-5)' }}>
        <div className="spread" style={{ marginBottom: 'var(--space-2)' }}>
          ...
          <span className="tiny muted">{STATUS_HINT[p.status]}</span>
        </div>
        <div
          style={{
            display: 'grid',
        ...
        >
          <div>
            <div className="tiny muted">Generation</div>
          ...
          <div>
            <div className="tiny muted">Finished</div>
            <span className="tiny tabular">{formatDate(p.finishedAt)}</span>
          </div>
          {p.externalUrl && (
            <div>
              <div className="tiny muted">External</div>
        ...
        {p.lastError && (
          <div style={{ marginTop: 'var(--space-4)', padding: 'var(--space-3)', background: 'rgba(239,68,68,0.08)', borderRadius: 'var(--radius-sm)', border: '1px solid rgba(239,68,68,0.2)' }}>
            <div className="tiny" style={{ color: 'var(--danger)', fontWeight: 600, marginBottom: 4 }}>last error</div>
      ...
      {p.events && p.events.length > 0 && (
        <div className="card">
          ...
          <p className="tiny muted" style={{ marginTop: 'var(--space-1)', marginBottom: 'var(--space-4)' }}>
            Every status transition writes one immutable event row. {p.events.length} event(s).
          </p>
